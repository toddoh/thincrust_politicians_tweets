...
filepath_dict = {'harris': './tweetdata/harris_12032009-06012019.csv'}
print('Dough: Init r11-0613_2019-harris')
...
def preprocess_tweets(startDate=None, endDate=None):
    for source, filepath in filepath_dict.items():
        ...
        for line in reader:
            ...
            if data['tweet'] in tweets_dup_check:
                print('Dough: Duplicate tweet bypass')
                continue

            tweets_dup_check.append(data['tweet'])
...
'''
tweets_init = preprocess_tweets(None, '01/12/2015')
tweets_type = 'before2016SenCampaign'
'''
tweets_init = preprocess_tweets('01/12/2015', None)
tweets_type = '2016Sencampaign-2019'
...
stop_words.extend(['kamala', 'harris', 'kamalaharris', 'facebook', 'album', 'rt', 'pm', 'yesterday', 'afternoon', 'tbt', 'tonight', 'click', 'join', 'day', 'news', 'just', 'great', 'live', 'thank', 'people', 'today', 'watch', 'posted', 'photo', 'photos']) #custom per-type
...
model_list, coherence_values = compute_coherence_values(dictionary=id2word, corpus=tweets_lemmatized_corpustdf, texts=tweets_lemmatized, start=5, limit=40, step=5)

# Print the coherence scores
x = range(5, 40, 5)
for m, cv in zip(x, coherence_values):
    print("Dough: ", m, " topic(s), has coherence value of", round(cv, 4))
...
df_dominant_topic.to_csv('./output/harris/result_lda_full_' + tweets_source + '-' + tweets_type + '.csv', sep='\t', index=False)
df_dominant_topic.to_json('./output/harris/result_lda_full_' + tweets_source + '-' + tweets_type + '.json', orient='records')
print('Dough: Saved full dataset with topic data in ', ' ./output/harris/result_lda_full_' + tweets_source + '-' + tweets_type + '.csv')
...
sent_topics_sorteddf_mallet.to_csv('./output/harris/result_lda_top_' + tweets_source + '-' + tweets_type + '.csv', sep='\t', index=False)
sent_topics_sorteddf_mallet.to_json('./output/harris/result_lda_top_' + tweets_source + '-' + tweets_type + '.json', orient='records')
print('Dough: Saved top 10 document dataset in ', ' ./output/harris/result_lda_top_' + tweets_source + '-' + tweets_type + '.csv')